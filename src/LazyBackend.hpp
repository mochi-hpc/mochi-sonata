...
class LazyBackend : public Backend {
    ...
    virtual RequestResult<uint64_t> storeJson(
        ...
        m_pool.make_thread([backend=this, coll_name, r=std::move(record), commit]() {
    ...
    virtual RequestResult<std::vector<uint64_t>> storeMultiJson(
        ...
        result.success() = true;
        result.value() = std::vector<uint64_t>(records.size(), std::numeric_limits<uint64_t>::max());
        // TODO check that collection exists
        m_pool.make_thread([backend=this, coll_name, r=std::move(records), commit]() {
            PendingWrite pw(*backend);
    ...
    virtual RequestResult<std::string> fetch(
        ...
        if(m_flush_on_read) flush(coll_name);
    ...
    struct PendingWrite {
        ...
        PendingWrite(LazyBackend& backend)
        : m_backend(backend) {
        ...
        ~PendingWrite() {
            ...
            if(notify) m_backend.m_pending_writes_cv.notify_all();
        }
    };
};
