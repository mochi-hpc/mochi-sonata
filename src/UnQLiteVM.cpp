...
#define LOG_ERROR()                                                            \
  unqlite_context_output_format(pCtx, "%s threw an exception: %s",             \
                                unqlite_result_null(pCtx), ex.what());

#define CHECK_ARGS(__num__)                                                    \
  do {                                                                         \
    if (argc != __num__) {                                                     \
      unqlite_context_output_format(                                           \
          pCtx, "%s: unexpected number of aruments (%d, expected %d)",         \
...
#define CHECK_ARGS2(__num1__, __num2__)                                        \
  do {                                                                         \
    if (argc != (__num1__) && argc != (__num2__)) {                            \
      unqlite_context_output_format(                                           \
          pCtx, "%s: unexpected number of aruments (%d, expected %d or %d)",   \
...
#define CATCH_AND_ABORT()                                                      \
    ...
    unqlite_context_output_format(pCtx, "%s threw an exception: %s",           \
                                  unqlite_function_name(pCtx), ex.what());     \
    unqlite_result_null(pCtx);                                                 \
    return UNQLITE_OK;                                                         \
  }

template <typename ResultType>
void test_or_wait_completion(unqlite_context *pCtx, bool wait,
                             AsyncRequest *req, ResultType *result) {
  // TODO
  if (wait) {
    delete result;
  } else {
...
struct collection_info {
  ...
  collection_info(unqlite_value *v) {
    ...
    if (!db_info_u) {
      throw Exception(
          "Could not find \"database\" entry in collection information");
    }
...
int UnQLiteVM::snta_db_create(unqlite_context *pCtx, int argc,
                              unqlite_value **argv) {
  UnQLiteVM *vm =
      reinterpret_cast<UnQLiteVM *>(unqlite_context_user_data(pCtx));
  CHECK_ARGS2(5, 6);
  // get parameters
  ...
  std::string token =
      (argc == 6) ? unqlite_value_to_string(argv[5], nullptr) : "";
  // call Admin function
  try {
    vm->m_backend->m_admin.createDatabase(address, provider_id, db_name,
                                          db_type, config.str(), token);
...
int UnQLiteVM::snta_db_attach(unqlite_context *pCtx, int argc,
                              unqlite_value **argv) {
  UnQLiteVM *vm =
      reinterpret_cast<UnQLiteVM *>(unqlite_context_user_data(pCtx));
  CHECK_ARGS2(5, 6);
  ...
  // call Admin function
  try {
    vm->m_backend->m_admin.attachDatabase(address, provider_id, db_name,
                                          db_type, config.str(), token);
    unqlite_result_bool(pCtx, true);
  }
  CATCH_AND_ABORT();
...
int UnQLiteVM::snta_db_detach(unqlite_context *pCtx, int argc,
                              unqlite_value **argv) {
  UnQLiteVM *vm =
      reinterpret_cast<UnQLiteVM *>(unqlite_context_user_data(pCtx));
  CHECK_ARGS2(3, 4);
  ...
  std::string db_name(unqlite_value_to_string(argv[2], nullptr));
  std::string token =
      (argc == 4) ? unqlite_value_to_string(argv[3], nullptr) : "";
  // call Admin function
  try {
    vm->m_backend->m_admin.detachDatabase(address, provider_id, db_name, token);
    unqlite_result_bool(pCtx, true);
  }
  CATCH_AND_ABORT();
  return UNQLITE_OK;
...
int UnQLiteVM::snta_db_destroy(unqlite_context *pCtx, int argc,
                               unqlite_value **argv) {
  UnQLiteVM *vm =
      reinterpret_cast<UnQLiteVM *>(unqlite_context_user_data(pCtx));
  CHECK_ARGS2(3, 4);
  ...
  // call Admin function
  try {
    vm->m_backend->m_admin.destroyDatabase(address, provider_id, db_name,
                                           token);
    unqlite_result_bool(pCtx, true);
  }
  CATCH_AND_ABORT();
...
int UnQLiteVM::sntd_coll_create(unqlite_context *pCtx, int argc,
                                unqlite_value **argv) {
  UnQLiteVM *vm =
      reinterpret_cast<UnQLiteVM *>(unqlite_context_user_data(pCtx));
  CHECK_ARGS(2);
  try {
    ...
    Database db = vm->m_backend->m_client.open(
...
int UnQLiteVM::sntd_coll_exists(unqlite_context *pCtx, int argc,
                                unqlite_value **argv) {
  UnQLiteVM *vm =
      reinterpret_cast<UnQLiteVM *>(unqlite_context_user_data(pCtx));
  CHECK_ARGS(2);
...
int UnQLiteVM::sntd_coll_open(unqlite_context *pCtx, int argc,
                              unqlite_value **argv) {
  UnQLiteVM *vm =
      reinterpret_cast<UnQLiteVM *>(unqlite_context_user_data(pCtx));
  CHECK_ARGS(2);
...
int UnQLiteVM::sntd_coll_drop(unqlite_context *pCtx, int argc,
                              unqlite_value **argv) {
  UnQLiteVM *vm =
      reinterpret_cast<UnQLiteVM *>(unqlite_context_user_data(pCtx));
  CHECK_ARGS(2);
...
int UnQLiteVM::sntd_execute(unqlite_context *pCtx, int argc,
                            unqlite_value **argv) {
  UnQLiteVM *vm =
      reinterpret_cast<UnQLiteVM *>(unqlite_context_user_data(pCtx));
  CHECK_ARGS2(2, 3);
  try {
    ...
    if (unqlite_value_is_callable(argv[1])) {
      ...
      if (code.empty()) {
        throw Exception("Could not find source code for function "s +
                        function_name);
      }
    ...
    if (argc == 3) {
      std::vector<std::string> variables =
          UnQLiteValue(argv[2], nullptr, nullptr, true);
...
int UnQLiteVM::sntc_store(unqlite_context *pCtx, int argc,
                          unqlite_value **argv) {
  UnQLiteVM *vm =
      reinterpret_cast<UnQLiteVM *>(unqlite_context_user_data(pCtx));
  CHECK_ARGS2(2, 3);
  try {
    ...
    if (unqlite_value_is_empty(argv[1]) || unqlite_value_is_resource(argv[1]) ||
...
int UnQLiteVM::sntc_fetch(unqlite_context *pCtx, int argc,
                          unqlite_value **argv) {
  UnQLiteVM *vm =
      reinterpret_cast<UnQLiteVM *>(unqlite_context_user_data(pCtx));
  CHECK_ARGS2(2, 3);
...
int UnQLiteVM::sntc_filter(unqlite_context *pCtx, int argc,
                           unqlite_value **argv) {
  UnQLiteVM *vm =
      reinterpret_cast<UnQLiteVM *>(unqlite_context_user_data(pCtx));
  CHECK_ARGS2(2, 3);
...
int UnQLiteVM::sntc_update(unqlite_context *pCtx, int argc,
                           unqlite_value **argv) {
  UnQLiteVM *vm =
      reinterpret_cast<UnQLiteVM *>(unqlite_context_user_data(pCtx));
  CHECK_ARGS2(3, 4);
  try {
    ...
    if (unqlite_value_is_empty(argv[2]) || unqlite_value_is_resource(argv[2]) ||
...
int UnQLiteVM::sntc_last_record_id(unqlite_context *pCtx, int argc,
                                   unqlite_value **argv) {
  UnQLiteVM *vm =
      reinterpret_cast<UnQLiteVM *>(unqlite_context_user_data(pCtx));
  CHECK_ARGS(1);
...
int UnQLiteVM::sntc_size(unqlite_context *pCtx, int argc,
                         unqlite_value **argv) {
  UnQLiteVM *vm =
      reinterpret_cast<UnQLiteVM *>(unqlite_context_user_data(pCtx));
  CHECK_ARGS(1);
...
int UnQLiteVM::sntc_erase(unqlite_context *pCtx, int argc,
                          unqlite_value **argv) {
  UnQLiteVM *vm =
      reinterpret_cast<UnQLiteVM *>(unqlite_context_user_data(pCtx));
  CHECK_ARGS2(2, 3);
...
int UnQLiteVM::sntr_wait(unqlite_context *pCtx, int argc,
                         unqlite_value **argv) {
  UnQLiteVM *vm =
      reinterpret_cast<UnQLiteVM *>(unqlite_context_user_data(pCtx));
  CHECK_ARGS(1);
  try {
    ...
    delete completion_fn; // XXX we should use placement new/delete to allocate
                          // in the context
...
int UnQLiteVM::sntr_test(unqlite_context *pCtx, int argc,
                         unqlite_value **argv) {
  UnQLiteVM *vm =
      reinterpret_cast<UnQLiteVM *>(unqlite_context_user_data(pCtx));
  CHECK_ARGS(1);
