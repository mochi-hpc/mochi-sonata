...
public:
  UnQLiteValue(unqlite_value *val, unqlite_vm *vm, unqlite_context *ctx,
               bool is_ref = false)
      : m_value(val), m_vm(vm), m_ctx(ctx), m_ref(is_ref) {}

  struct Null {};

  UnQLiteValue(UnQLiteValue &&other)
      : m_vm(other.m_vm), m_value(other.m_value), m_ctx(other.m_ctx) {
    other.m_vm = nullptr;
  ...
  UnQLiteValue(unqlite_vm *vm) : m_vm(vm), m_value(unqlite_vm_new_scalar(vm)) {
    unqlite_value_null(m_value);
  }

  UnQLiteValue(unqlite_context *ctx)
      : m_ctx(ctx), m_value(unqlite_context_new_scalar(ctx)) {
    unqlite_value_null(m_value);
  }
  ...
  UnQLiteValue(const Null &, unqlite_vm *vm)
      : m_vm(vm), m_value(unqlite_vm_new_scalar(vm)) {
    unqlite_value_null(m_value);
  }
  ...
  UnQLiteValue(const std::vector<T> &vec, unqlite_vm *vm)
      : m_vm(vm), m_value(unqlite_vm_new_array(vm)) {
  ...
  UnQLiteValue(const std::vector<T> &vec, unqlite_context *ctx)
      : m_ctx(ctx), m_value(unqlite_context_new_array(ctx)) {
    for (const auto &val : vec) {
  ...
  UnQLiteValue(const std::pair<UnQLiteValue, UnQLiteValue> &p,
               unqlite_context *ctx)
      : m_ctx(ctx), m_value(unqlite_context_new_array(ctx)) {
    unqlite_array_add_elem(m_value, nullptr, p.first.m_value);
    unqlite_array_add_elem(m_value, nullptr, p.second.m_value);
  ...
  template <typename T>
  UnQLiteValue(const std::unordered_map<std::string, T> &values,
               unqlite_context *ctx)
      : m_ctx(ctx), m_value(unqlite_context_new_array(ctx)) {
    for (const auto &p : values) {
      UnQLiteValue uval(p.second, ctx);
  ...
  template <typename T>
  UnQLiteValue(const std::map<std::string, UnQLiteValue> &values,
               unqlite_vm *vm)
      : m_vm(vm), m_value(unqlite_vm_new_array(vm)) {
    for (const auto &p : values) {
  ...
  template <typename T>
  UnQLiteValue(const std::unordered_map<std::string, UnQLiteValue> &values,
               unqlite_vm *vm)
      : m_vm(vm), m_value(unqlite_vm_new_array(vm)) {
    for (const auto &p : values) {
      unqlite_array_add_strkey_elem(m_value, p.first.c_str(), p.second.m_value);
  ...
  size_t size() const { return unqlite_array_count(m_value); }
  ...
  UnQLiteValue operator[](int index) const {
    ...
    if (result) {
      ...
      if (m_vm)
        ...
        return UnQLiteValue(Null(), m_ctx);
    }
  }
  ...
  template <typename T> T as() const { return convertType(m_value, type<T>()); }
  ...
  template <typename Stream> Stream &printToStream(Stream &os) const {
    if (is<std::string>()) {
  ...
  void foreach (
      const std::function<void(const std::string &, const UnQLiteValue &)> &f) {
    foreach (m_value, [vm = m_vm, ctx = m_ctx, &f](const std::string &key,
                                                   unqlite_value *elem) {
      UnQLiteValue val(elem, vm, ctx, true);
      f(key, val);
      return UNQLITE_OK;
...
private:
  static int foreachArrayCallback(unqlite_value *pKey, unqlite_value *pValue,
                                  void *pUserData) {
    using arg_type =
        std::pair<unsigned, std::function<int(unsigned, unqlite_value *)> *>;
  ...
  static int foreachMapCallback(unqlite_value *pKey, unqlite_value *pValue,
                                void *pUserData) {
  ...
  friend int printUnQLiteVec(unqlite_value *pKey, unqlite_value *pValue,
                             void *pUserData);

  template <typename Stream>
  friend int printUnQLiteMap(unqlite_value *pKey, unqlite_value *pValue,
                             void *pUserData);

  template <typename T1> void fillArrayFromTuple(T1 &&x) {
    if (m_vm) {
  ...
  template <typename IntegerType,
            std::enable_if_t<std::is_integral<IntegerType>::value, int> = 0>
  static IntegerType convertType(unqlite_value *value,
  ...
  static std::string convertType(unqlite_value *value,
                                 const type<std::string> &) {
    return unqlite_value_to_string(value, NULL);
  }

  template <typename T>
  static std::vector<T> convertType(unqlite_value *value,
                                    const type<std::vector<T>> &) {
    std::vector<T> result;
    foreach (value, [&result](unsigned, unqlite_value *pValue) {
      result.push_back(
          UnQLiteValue::convertType(pValue, UnQLiteValue::type<T>()));
      return UNQLITE_OK;
    })
      ;
    return result;
  ...
  convertType(unqlite_value *value, const type<std::map<std::string, T>> &) {
  ...
  convertType(unqlite_value *value,
              const type<std::unordered_map<std::string, T>> &) {
  ...
  template <typename T>
  static bool checkType(unqlite_value *value,
                        const type<std::map<std::string, T>> &) {
