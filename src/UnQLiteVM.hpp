...
public:
  UnQLiteVM(unqlite *database, const char *code, UnQLiteBackend *backend)
      : m_code(code), m_db(database), m_backend(backend) {
    compile();
  }
  ...
  ~UnQLiteVM() { unqlite_vm_release(m_vm); }
  ...
  void execute(const std::function<int(const void *, unsigned int)> &callback) {
    int ret = unqlite_vm_config(
        m_vm, UNQLITE_VM_CONFIG_OUTPUT, output_callback,
        const_cast<void *>(reinterpret_cast<const void *>(&callback)));
    if (ret != UNQLITE_OK)
      parse_and_throw_error();
    execute();
  }
  ...
  template <typename T> void set(const std::string &name, const T &value) {
    ...
    int ret =
        unqlite_vm_config(m_vm, UNQLITE_VM_CONFIG_CREATE_VAR,
  ...
  std::string output() const {
    const char *pOut;
    unsigned int nLen;
    unqlite_vm_config(m_vm, UNQLITE_VM_CONFIG_EXTRACT_OUTPUT, &pOut, &nLen);
  ...
  void registerSonataFunctions() {
    int ret;
    ret = unqlite_create_constant(m_vm, "__SCRIPT__", get_script,
                                  static_cast<void *>(this));
    // Admin functions
    ret = unqlite_create_function(m_vm, "sonata_db_create", snta_db_create,
                                  static_cast<void *>(this));
    ret = unqlite_create_function(m_vm, "sonata_db_attach", snta_db_attach,
                                  static_cast<void *>(this));
    ret = unqlite_create_function(m_vm, "sonata_db_detach", snta_db_detach,
                                  static_cast<void *>(this));
    ret = unqlite_create_function(m_vm, "sonata_db_destroy", snta_db_destroy,
                                  static_cast<void *>(this));
    // Database functions
    ret = unqlite_create_function(m_vm, "sonata_coll_create", sntd_coll_create,
                                  static_cast<void *>(this));
    ret = unqlite_create_function(m_vm, "sonata_coll_exists", sntd_coll_exists,
                                  static_cast<void *>(this));
    ret = unqlite_create_function(m_vm, "sonata_coll_open", sntd_coll_open,
                                  static_cast<void *>(this));
    ret = unqlite_create_function(m_vm, "sonata_coll_drop", sntd_coll_drop,
                                  static_cast<void *>(this));
    ret = unqlite_create_function(m_vm, "sonata_execute", sntd_execute,
                                  static_cast<void *>(this));
    // Collection functions
    ret = unqlite_create_function(m_vm, "sonata_store", sntc_store,
                                  static_cast<void *>(this));
    ret = unqlite_create_function(m_vm, "sonata_fetch", sntc_fetch,
                                  static_cast<void *>(this));
    ret = unqlite_create_function(m_vm, "sonata_filter", sntc_filter,
                                  static_cast<void *>(this));
    ret = unqlite_create_function(m_vm, "sonata_update", sntc_update,
                                  static_cast<void *>(this));
    ret = unqlite_create_function(m_vm, "sonata_all", sntc_all,
                                  static_cast<void *>(this));
    ret =
        unqlite_create_function(m_vm, "sonata_last_record_id",
                                sntc_last_record_id, static_cast<void *>(this));
    ret = unqlite_create_function(m_vm, "sonata_size", sntc_size,
                                  static_cast<void *>(this));
    ret = unqlite_create_function(m_vm, "sonata_erase", sntc_erase,
                                  static_cast<void *>(this));
    // Wait functions
    ret = unqlite_create_function(m_vm, "sonata_wait", sntr_wait,
                                  static_cast<void *>(this));
    ret = unqlite_create_function(m_vm, "sonata_test", sntr_test,
                                  static_cast<void *>(this));
  }
...
private:
  ...
  std::string extract_function_code(const char *function_name,
    ...
    while ((tmp = strstr(tmp, "function")) != NULL) {
      ...
      if (include_fun_decl) {
        return std::string(function_beginning,
                           (size_t)(function_end - function_beginning));
      } else {
        return std::string(
            function_body_beginning,
            (size_t)(function_body_end - function_body_beginning));
      }
    }
    return std::string();
  }
  ...
  static int output_callback(const void *pOutput, unsigned int nOutputLen,
                             void *pUserData) {
    auto function_ptr =
        static_cast<std::function<int(const void *, unsigned in)> *>(pUserData);
    if (function_ptr) {
  ...
  static int snta_db_create(unqlite_context *pCtx, int argc,
                            unqlite_value **argv);
  static int snta_db_attach(unqlite_context *pCtx, int argc,
                            unqlite_value **argv);
  static int snta_db_detach(unqlite_context *pCtx, int argc,
                            unqlite_value **argv);
  static int snta_db_destroy(unqlite_context *pCtx, int argc,
                             unqlite_value **argv);

  static int sntd_coll_create(unqlite_context *pCtx, int argc,
                              unqlite_value **argv);
  static int sntd_coll_exists(unqlite_context *pCtx, int argc,
                              unqlite_value **argv);
  static int sntd_coll_open(unqlite_context *pCtx, int argc,
                            unqlite_value **argv);
  static int sntd_coll_drop(unqlite_context *pCtx, int argc,
                            unqlite_value **argv);
  static int sntd_execute(unqlite_context *pCtx, int argc,
                          unqlite_value **argv);
  ...
  static int sntc_last_record_id(unqlite_context *pCtx, int argc,
                                 unqlite_value **argv);
