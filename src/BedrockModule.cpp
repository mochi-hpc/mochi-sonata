...
class SonataFactory : public bedrock::AbstractServiceFactory {
    ...
    void* registerProvider(const bedrock::FactoryArgs& args) override {
        auto provider = new sonata::Provider(
                args.mid, args.provider_id, tl::pool(args.pool));
    ...
    void* createProviderHandle(void* c, hg_addr_t address, uint16_t provider_id) override {
        ...
        auto ph = new sonata::ProviderHandle(client->createProviderHandle(address, provider_id));
