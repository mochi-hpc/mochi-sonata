...
public:
  ...
  createCollection(const std::string &coll_name) = 0;
   ...
   * @return a RequestResult<bool> instance.
   */
  virtual RequestResult<bool> openCollection(const std::string &coll_name) = 0;
   ...
   * @return a RequestResult<bool> instance.
   */
  virtual RequestResult<bool> dropCollection(const std::string &coll_name) = 0;
  ...
  storeMulti(const std::string &coll_name,
             const std::vector<std::string> &records, bool commit) = 0;
   ...
   */
  virtual RequestResult<std::vector<std::string>>
  filter(const std::string &coll_name, const std::string &filter_code) = 0;
  ...
  updateMulti(const std::string &coll_name,
              const std::vector<uint64_t> &record_ids,
              const std::vector<std::string> &new_contents, bool commit) = 0;
   ...
   */
  virtual RequestResult<std::vector<std::string>>
  all(const std::string &coll_name) = 0;
   ...
   */
  virtual RequestResult<uint64_t> lastID(const std::string &coll_name) = 0;
   ...
   */
  virtual RequestResult<size_t> size(const std::string &coll_name) = 0;
  ...
  virtual RequestResult<bool> erase(const std::string &coll_name,
                                    uint64_t record_id, bool commit) = 0;
  ...
  eraseMulti(const std::string &coll_name,
             const std::vector<uint64_t> &record_ids, bool commit) = 0;
...
class BackendFactory {
  ...
  template <typename BackendType> friend class ::__SonataBackendRegistration;
...
public:
   ...
   * @param engine Thallium engine.
   * @param pool Pool used by the underlying provider.
   * @param config Configuration object to pass to the backend's attach
   * function.
   *
   * @return a unique_ptr to the created Backend.
   */
...
#define SONATA_REGISTER_BACKEND(__backend_name, __backend_type)                \
  static __SonataBackendRegistration<__backend_type>                           \
      __sonata##__backend_name##_backend(#__backend_name)
